...
def logistic_regression(train,validation):
    ...
    print "Runnin  time logistic regression is ", stop-start
    ...
    print "Log loss is ",log_loss(validation['Category'], predicted)
    Category_new=[]
    for i in range(0,len(model1)):
        Category_new.append(le_crime.classes_[model1[i]])
    result1=pd.DataFrame(predicted, columns=le_crime.classes_)
    result1['Predicted']=Category_new
    result1.to_csv('logisticRegr_test.csv', index = True, index_label = 'Id' )
...
#print " ",set(Hour)," ",set(Minute)," ",set(Seasons)," ",set(Time)
